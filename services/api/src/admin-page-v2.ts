...
<body>
  <main>
    ...
    <section class="panel">
      <div class="controls">
        ...
        <select id="view"><option value="todo">À traiter</option><option value="all">Tous actifs</option><option value="archived">Archives</option><option value="custom">Statut précis</option></select>
        <select id="status"><option value="">Tous les statuts</option><option value="pending">Pending</option><option value="pending_review">Pending review</option><option value="approved">Approved</option><option value="publishing">Publishing</option><option value="published">Published</option><option value="rejected">Rejected</option><option value="failed">Failed</option><option value="archived">Archived</option></select>
        <select id="platform"><option value="">Toutes plateformes</option><option value="x">X</option><option value="linkedin">LinkedIn</option><option value="facebook">Facebook</option><option value="instagram">Instagram</option></select>
        <select id="order"><option value="desc">Plus récent en haut</option><option value="asc">Plus récent en bas</option></select>
  ...
  <script>
    ...
    function updateStatusAvailability() { statusInput.disabled = viewInput.value !== 'custom'; if (viewInput.value !== 'custom') statusInput.value = ''; }
    ...
    function preview(platform, content) {
      var text = String(content || '').trim();
      var messages = [];
      var level = 'ok';
      if (!text) { level = 'error'; messages.push('Contenu vide.'); }
      if (text.includes('/publish') || text.includes('#publish') || text.includes('#relaypress')) { if (level !== 'error') level = 'warn'; messages.push('Commande ou tag de routage encore visible.'); }
      if (platform === 'x' && count(text) > 140) { if (level !== 'error') level = 'warn'; messages.push('X dépasse 140 caractères.'); }
      if (platform === 'instagram') { if (level !== 'error') level = 'warn'; messages.push('Instagram : média non géré pour le moment.'); }
      if (!messages.length) messages.push('OK pour validation éditoriale mock.');
      return '<div class="preview ' + esc(level) + '"><strong>Prévisualisation ' + esc(platform) + '</strong> · ' + esc(countLabel(text)) + messages.map(function (message) { return '<div class="preview-message">' + esc(message) + '</div>'; }).join('') + '<div class="content">' + esc(text || '—') + '</div></div>';
    }
    ...
    function canReadapt(job) { return canEdit(job); }

    async function createManualDraft() {
      var content = draftContent.value.trim();
      var platforms = Array.from(document.querySelectorAll('input[name="draftPlatform"]:checked')).map(function (input) { return input.value; });
      if (!content) return alert('Le brouillon est vide.');
      if (!platforms.length) return alert('Sélectionne au moins une plateforme.');
      draftStatus.textContent = 'Création…';
      try {
        var payload = await api('/publication-jobs/manual-draft', { method: 'POST', body: JSON.stringify({ content: content, platforms: platforms }) });
        draftStatus.textContent = payload.count + ' job(s) créés.';
        draftContent.value = '';
    ...
    async function action(id, suffix, confirmText) {
      if (confirmText && !confirm(confirmText)) return;
      await api(jobUrl(id, suffix), { method: 'POST' });
      await loadJobs();
    }
    ...
    async function archiveSelected() {
      ...
      if (!confirm('Archiver ' + ids.length + ' job(s) sélectionné(s) ?')) return;
      var ok = 0;
      var fail = 0;
      for (var i = 0; i < ids.length; i += 1) {
        try { await api(jobUrl(ids[i], '/archive'), { method: 'POST' }); ok += 1; } catch (_error) { fail += 1; }
      }
      statusLine.textContent = ok + ' archivé(s), ' + fail + ' erreur(s).';
      await loadJobs();
    }

    function renderJob(job) {
      var original = sourceFor(job);
      var adapted = String(job.adaptedContent || '').trim();
      var editorId = 'editor-' + job.id.replace(/[^a-zA-Z0-9]/g, '-');
      var runsId = 'runs-' + job.id.replace(/[^a-zA-Z0-9]/g, '-');
      var differs = original !== adapted;
      var html = '';
      html += '<article class="job">';
      html += '<div class="job-header"><div class="badges">';
      if (canArchive(job)) html += '<label class="badge"><input type="checkbox" name="jobSelect" value="' + esc(job.id) + '" /> archiver</label>';
      html += '<span class="badge ' + esc(job.status) + '">' + esc(job.status) + '</span><span class="badge">' + esc(job.platform) + '</span><span class="badge">' + esc(job.sourceEventId ? 'nostr' : 'manual') + '</span>';
      html += '</div><div class="meta">Créé: ' + date(job.createdAt) + '<br/>Mis à jour: ' + date(job.updatedAt) + '</div></div>';
      html += '<details ' + (differs ? 'open' : '') + '><summary>Comparer source originale / version adaptée</summary><div class="compare"><div class="box"><div class="box-title">Source originale <span class="count">' + esc(countLabel(original)) + '</span></div><div class="content">' + esc(original || '—') + '</div></div><div class="box adapted"><div class="box-title">Version adaptée <span class="count" data-count="adapted">' + esc(countLabel(adapted)) + '</span></div><div class="content" data-content="adapted">' + esc(adapted || '—') + '</div></div></div><div class="hint">' + (differs ? 'Les deux contenus diffèrent : vérifie que l’adaptation respecte l’intention source.' : 'Source et version adaptée sont identiques ou proches.') + '</div></details>';
      if (canEdit(job)) html += '<textarea id="' + editorId + '">' + esc(adapted) + '</textarea>';
      html += '<div data-preview="job">' + preview(job.platform, adapted) + '</div>';
      html += '<div class="meta"><strong>Job:</strong> ' + esc(job.id) + '<br/><strong>External:</strong> ' + esc(job.externalPostId || '—') + '<br/><strong>Erreur:</strong> ' + esc(job.errorMessage || '—') + '</div>';
      html += '<div class="actions">';
      if (canEdit(job)) html += '<button class="primary" data-action="save">Enregistrer le texte</button>';
      if (canReadapt(job)) html += '<button data-action="readapt">Réadapter depuis la source</button>';
      ...
      if (canReset(job)) html += '<button data-action="reset">Remettre en review</button>';
      ...
      html += '<button data-action="runs">Voir les runs</button><button data-action="copy">Copier ID</button>';
      html += '</div><details><summary>Détails source</summary><pre>' + esc(JSON.stringify(job.sourceEvent, null, 2)) + '</pre></details><details><summary>Historique d’exécution</summary><pre id="' + runsId + '">Clique sur “Voir les runs”.</pre></details></article>';
      ...
      if (editor) editor.addEventListener('input', function () {
        ...
        var previewTarget = card.querySelector('[data-preview="job"]');
        var contentTarget = card.querySelector('[data-content="adapted"]');
        var countTarget = card.querySelector('[data-count="adapted"]');
        if (previewTarget) previewTarget.innerHTML = preview(job.platform, value);
        if (contentTarget) contentTarget.textContent = value.trim() || '—';
        if (countTarget) countTarget.textContent = countLabel(value);
      });
      var select = card.querySelector('input[name="jobSelect"]');
      if (select) select.addEventListener('change', updateSelectionStatus);
      var save = card.querySelector('[data-action="save"]');
      if (save) save.addEventListener('click', function () { updateContent(job.id, editor.value).catch(function (error) { alert(error.message); }); });
      var readapt = card.querySelector('[data-action="readapt"]');
      if (readapt) readapt.addEventListener('click', function () { action(job.id, '/readapt', 'Réadapter depuis la source originale ? Les modifications manuelles actuelles seront remplacées.').catch(function (error) { alert(error.message); }); });
      var approve = card.querySelector('[data-action="approve"]');
      if (approve) approve.addEventListener('click', function () { action(job.id, '/approve').catch(function (error) { alert(error.message); }); });
      var retry = card.querySelector('[data-action="retry"]');
      if (retry) retry.addEventListener('click', function () { action(job.id, '/retry', 'Relancer ce job en publication ?').catch(function (error) { alert(error.message); }); });
      var reset = card.querySelector('[data-action="reset"]');
      if (reset) reset.addEventListener('click', function () { action(job.id, '/reset-review', 'Remettre ce job en pending_review ?').catch(function (error) { alert(error.message); }); });
      var reject = card.querySelector('[data-action="reject"]');
      if (reject) reject.addEventListener('click', function () { rejectJob(job.id).catch(function (error) { alert(error.message); }); });
      var archive = card.querySelector('[data-action="archive"]');
      if (archive) archive.addEventListener('click', function () { action(job.id, '/archive', 'Archiver ce job ?').catch(function (error) { alert(error.message); }); });
      var runs = card.querySelector('[data-action="runs"]');
      if (runs) runs.addEventListener('click', function () { loadRuns(job.id, card.querySelector('#' + CSS.escape(runsId))); });
      var copy = card.querySelector('[data-action="copy"]');
      if (copy) copy.addEventListener('click', function () { navigator.clipboard.writeText(job.id); statusLine.textContent = 'ID copié.'; });
      return card;
    }
    ...
    async function loadJobs() {
      ...
      try {
        ...
        statusLine.textContent = payload.count + ' job(s).';
        if (!payload.jobs.length) {
          jobsEl.innerHTML = '<div class="panel empty">Aucun job pour ces filtres.</div>';
          updateSelectionStatus();
          return;
        }
        payload.jobs.forEach(function (job) { jobsEl.appendChild(renderJob(job)); });
        updateSelectionStatus();
      } catch (error) {
