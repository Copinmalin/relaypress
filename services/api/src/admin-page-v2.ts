...
<body>
  ...
  <script>
    ...
    async function connectLinkedIn() { try { var payload = await api('/publisher-accounts/linkedin/oauth/start', { method: 'POST' }); window.location.href = payload.authorizationUrl; } catch (error) { alert(error.message); } }
    tokenInput.addEventListener('input', function () { localStorage.setItem('relaypress.adminToken', tokenInput.value.trim()); updateTokenHint(); });
