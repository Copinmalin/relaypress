...
const statuses = new Set<EditorialSignalStatus>([
  "qualified",
  "needs_sources",
  "ready_for_campaign",
  "ignored",
...
export async function registerEditorialSignalRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post<{ Params: SourceItemSignalParams; Body: CreateEditorialSignalBody }>(
    ...
    async (request, reply) => {
      ...
      if (!category || !summaryInternal || !editorialAngle) {
        return reply.code(400).send({
          error: "invalid_signal",
          message: "Category, internal summary and editorial angle are required",
        });
      }
      if (!isRisk(riskLevel)) {
      ...
      if (!(await sourceExists(request.params.id))) {
      ...
      await pool.query(
        ...
        [
          ...
          riskLevel,
          status,
          JSON.stringify(cleanSources(request.body?.primarySources)),
        ],
      );
  ...
  app.post<{ Params: EditorialSignalParams; Body: UpdateEditorialSignalStatusBody }>(
    ...
    async (request, reply) => {
      ...
      const result = await pool.query(
        "update editorial_signals set status = $2, updated_at = now() where id = $1 returning id",
